...
The second hierarchical equality model takes in input vectors, $a,b,c,d$, and applies the simple equality model from equations \dasheg{eq:x2h}{eq:h2y} to the pairs $(a,b)$
...
\end{equation}
%
for $N$ examples. Here, $T_{i}$ is the length of example $i$. As before, $\theta$ abbreviates the parameters of the model as specified in \dasheg{eq:lstm-recur-supp}{eq:lstm-predict-supp}. We use $h_{\theta}(x^{i, 0:{t-1}})$ for the vector predicted by the model for example~$i$ at timestep~$t$, which is compared to the actual vector at timestep $t$ via squared Euclidean distance.
